...
import com.yang.sunment.utils.BuildArticleTabloidUtil;
...
public class EditControl {
    ...
    public JSONObject publishArticle(@AuthenticationPrincipal Principal principal,
       ...
       /* String phone = secAccountService.findPhoneByUsername(username);
        if(!secAccountService.isSuperAdmin(phone)){
        ...
        article.setArticleAbstract(articleHtmlContent + "...");
   ...
   /* *//**
    ...
    @GetMapping("/isWrite")
    @ResponseBody
    public int isWrite(@AuthenticationPrincipal Principal principal){
    ...
    public JSONObject getDraftArticle(HttpServletRequest request){
        ...
        if(id != null){
            ...
            returnJson.put("result", articleService.getDraftArticle(article, articleTags, tagService.tagsSizeByTagName(articleTags[0])));
            return returnJson;
        }
        //判断是否为写文章登录超时
