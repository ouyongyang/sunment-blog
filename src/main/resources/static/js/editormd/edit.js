...
$(function() {
    testEditor = editormd("my-editormd", { //注意1：这里的就是上面的DIV的id属性值
        ...
        imageUploadURL : "/edit/uploadImage",
        onload:function () {
...
var articleTitle = $('#oyy-editor-title');
...
function publishSuccessPutIn(data) {
    ...
    var successBtn = $('<div class="successBtn">' +
        '<a href="/jump/edit" class="reWriteBtn am-btn am-btn-success am-round">写新文章</a>' +
        '<a href="' + data.articleUrl + '" class="lookArticleBtn am-btn am-btn-success am-round">查看文章</a>' +
        '</div>');
...
$.ajax({
    ...
    success:function (data) {
        if(data['status'] == 201){
            $('#oyy-editor-title').val(data['result']['articleTitle']);
